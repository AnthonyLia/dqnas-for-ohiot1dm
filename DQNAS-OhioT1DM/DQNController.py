...
from keras.layers import Dense, Dropout, Conv1D, MaxPooling1D, Activation, Flatten, Input, LSTM
...
class DQNAgent (CNNSearchSpace):
    ...
    def sample_architecture_sequences(self, model, number_of_samples):
        ...
        while len(samples) < number_of_samples:
            ...
            while len(seed) < self.max_len:
                ...
                #print("after len(sequence) is %d" %len(sequence))
                ...
                #print(probab)
                temp_max_index = list(probab[0]).index(max(probab[0]))
                #print(temp_max_index)
                #print(probab[0][temp_max_index-5:temp_max_index+5])
                #probab = probab[0][0]
                #print(probab[0][0])
                ...
                if self.EPSILON > 0:
                    ...
                    list_rep=probab[0].tolist()
                    next=np.array([vocab_idx[list_rep.index(best_action)]])
                    print('got to epsilon == 0')
                '''
                
                next = np.random.choice(vocab_idx, size=1, p=probab[0])
                print(f'next is {next}')'''
                #Here we have to specify a range of values, to cover the point of dropout cannot be the first layer 
                if ((next >= self.conv_id) and len(seed) == 0) or ( next == final_layer_id and len(seed) == 0):
                    while ((next >= self.conv_id) and len(seed) == 0) or ( next == final_layer_id and len(seed) == 0):
                        next = np.random.choice(vocab_idx, size=1, p=probab[0])
                    
                #Have to make a rule such that first layer cannot be anything except the Convolutional Layer

                '''   I, Anthony Liardo, implemented a combined check to not use 'continue' and have the model re-predict
                ...
                if next == final_layer_id:
                    if len(seed) < self.max_len:
                        print('got "next" to be final_layer_id but not on last element of seed')
                    seed.pop()
                    seed.append(np.array([Flatten_id]))
                    seed.append(np.array([next]))
                    break
                ...
                if len(seed) == self.max_len - 2:
                    seed.append(np.array([Flatten_id]))
                    seed.append(np.array([final_layer_id]))
                    sequence = pad_sequences([seed], maxlen=self.max_len - 1, padding='post')
                    sequence = sequence.reshape(1, 1, self.max_len - 1)
                    break
            ...
            if seed not in self.seq_data:
                #print('about to append seed: %s' %str(seed))
                samples.append(seed)
                self.seq_data.append(seed)
            else:
                self.EPSILON += 0.1
                print('wanted to append self.seq_data with seed but seed was already in seq_data, didnt append for this while loop')
        return samples    
    ...
    def train_control_model(self, model, target_model, x_data, y_data, mae_target, loss_func, controller_batch_size, nb_epochs):
        

        for i in range(len(x_data)):
            self.replay_memory.append([x_data[i][0],y_data[i],mae_target[i]])
        
        if len(self.replay_memory)<self.MIN_REPLAY_MEMORY_SIZE:
            return
        #Top 250 Architectures are taken
        self.replay_memory= sorted(self.replay_memory,key=itemgetter(2))
        to_train=self.replay_memory[:250]
        #optim = getattr(tensorflow.keras.optimizers, self.controller_optimizer)(lr=self.controller_lr, 
         #                                              decay=self.controller_decay)
        optim = getattr(tensorflow.keras.optimizers.legacy, self.controller_optimizer)(lr=self.controller_lr, 
                                                       decay=self.controller_decay)
                                                       
        # compile model depending on loss function and optimizer provided
        model.compile(optimizer=optim, loss={'main_output': loss_func}, run_eagerly=True)
        ...
        for i in range(len(to_train)):
            x_data.append(to_train[i][0].reshape(1,MAX_ARCHITECTURE_LENGTH-1))
            y_data.append(to_train[i][1])
            print(f'shape of y_data[i] is {y_data[i].shape}')
        
        # train the controller
        print(f'len(y_data[0]) is {len(y_data[0])}')
        ...
        print(f'for reference, x_data is {x_data} and y_data is {y_data}')
    ...
    def train_hybrid_model(self, model, target_model, x_data, y_data, val_accuracy, pred_accuracy, loss_func, controller_batch_size, nb_epochs):
        ...
        optim = getattr(tensorflow.keras.optimizers.legacy, self.controller_optimizer)(lr=self.controller_lr, decay=self.controller_decay, clipnorm=1.0)

        
        model.compile(optimizer=optim,
                      loss={'main_output': loss_func, 'predictor_output': 'mae'},
        ...
        for i in range(len(to_train)):
            ...
            print(f'y_data[i] is {y_data[i]}')

        print(f'y_data[0] is {y_data[0]}')
        print(f'for reference, x_data is {x_data} and y_data is {y_data}')
        print(f'also, pred_target is {pred_target}')
        print("TRAINING CONTROLLER...")
        model.fit({'main_input': np.array(x_data)},
                  {'main_output': np.array(y_data),
    ...
    def get_predicted_accuracies_hybrid_model(self, model, seqs):
        ...
        for seq in seqs:
            ...
            (_, pred_accuracy) = [x[0][0] for x in model.predict(xc,verbose='0')]
            #pred_accuracy = [x[0][0] for x in model.evaluate(xc) ]
