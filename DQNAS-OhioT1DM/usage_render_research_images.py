...
#------------------- Ground Truth vs. Predictions -------------------------#
    ...
    '''if patient == 552:
        hrs_552 = np.array(list(range(len(plot_preds.squeeze())))) / 12
        fig, axs = plt.subplots(2)
        axs[0].plot(hrs_552, plot_df.values[6:], color='black', linestyle='--', label='Ground Truth')
        axs[0].set_xlabel('Time (hours)')
        axs[0].set_ylabel('Blood Glucose Values (mg/dL)')
        axs[0].plot(hrs_552, plot_preds.squeeze(), color='blue', label='Predictions')
        axs[0].legend()
        axs[0].set_xlim(0,50)
        axs[0].text(25, -53, '(a)')
 
    if patient == 584:
        hrs_584 = np.array(list(range(len(plot_preds.squeeze())))) / 12
        axs[1].plot(hrs_584, plot_df.values[6:], color='black', linestyle='--', label='Ground Truth')
        axs[1].set_xlabel('Time (hours)')
        axs[1].set_ylabel('Blood Glucose Values (mg/dL)')
        axs[1].plot(hrs_584, plot_preds.squeeze(), color='blue', label='Predictions')
        axs[1].legend()
        axs[1].set_xlim(0,50)
        axs[1].text(25, -100, '(b)')
        plt.subplots_adjust( bottom = 0.2, hspace = 0.7)
        plt.show()
        fig.savefig('552-and-584-plots.pdf', dpi=500., pad_inches=0.1, bbox_inches='tight')'''
...
#------------------------------ SEG Plot ----------------------------------#
    ...
    '''if patient == 584:
        fig, ax = plt.subplots()
        img = plt.imread('seg_canvas.jpg')
        ax.imshow(img, extent=[0,600,0,600])axs[1].plot(hrs_584, plot_df.values[6:], color='black', linestyle='--', label='Ground Truth')
        axs[1].set_xlabel('Measured Blood Glucose Values (mg/dL)')
        axs[1].set_ylabel('Predicted Blood Glucose Values (mg/dL)')
        axs[1].plot(hrs_584, plot_preds.squeeze(), color='blue', label='Predictions')
        axs[1].legend()
        axs[1].set_xlim(0,50)
        axs[1].text(25, -100, '(b)')
